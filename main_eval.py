...
def evaluate_model(model, retrained_model, device, args):
    ...
    knn_accuracy = utils.evaluate_knn(model, args)
    
    results.update({
        'imagenet_forget_knn': knn_accuracy['forget'],
        'imagenet_retain_knn': knn_accuracy['retain'],
        'imagenet_val_forget_knn': knn_accuracy['val_forget'],
        'imagenet_val_retain_knn': knn_accuracy['val_retain'],
    ...
    print("Evaluating CKA on ImageNet datasets...")
...
def main():
    ...
    model_paths = [
        ...
        "./result/GA/GA/5e-6/GAcheckpoint.pth.tar",
        "./result/RL/RL_imagenet/5e-06/2/RL_imagenetcheckpoint.pth.tar",
        "./result/SalUn/RL_imagenet/5e-06/2/SalUncheckpoint.pth.tar",
    ...
    for model_path in model_paths:
        ...
        # evaluate model
        results = evaluate_model(model, retrained_model, device, args)
        ...
        # save results
        save_dir = f"results/evaluation/{model_path.split('/')[-3].replace('checkpoint','')}"
