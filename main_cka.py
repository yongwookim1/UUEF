...
def evaluate_cka(model, retrained_model, data_loader, device, mode='avgpool'):
    """
...
def main():
    ...
    model = utils.load_model(args.model_path, device).to(device)
    retrained_model = utils.load_model(args.retrained_model_path, device).to(device)
    model.eval()
    retrained_model.eval()
    ...
    office_home_real_world_results = evaluate_cka(model, retrained_model, office_home_real_world_data_loader, device)
    office_home_art_results = evaluate_cka(model, retrained_model, office_home_art_data_loader, device)
    office_home_clipart_results = evaluate_cka(model, retrained_model, office_home_clipart_data_loader, device)
    office_home_product_results = evaluate_cka(model, retrained_model, office_home_product_data_loader, device)
    
    cub_results = evaluate_cka(model, retrained_model, cub_data_loader, device)
    
    domainnet126_clipart_results = evaluate_cka(model, retrained_model, domainnet126_clipart_data_loader, device)
    domainnet126_painting_results = evaluate_cka(model, retrained_model, domainnet126_painting_data_loader, device)
    domainnet126_real_results = evaluate_cka(model, retrained_model, domainnet126_real_data_loader, device)
    domainnet126_sketch_results = evaluate_cka(model, retrained_model, domainnet126_sketch_data_loader, device)

    print(f"Office-Home Real World CKA: {office_home_real_world_results['cka']*100:.2f}")
    print(f"Office-Home Art CKA: {office_home_art_results['cka']*100:.2f}")
    print(f"Office-Home Clipart CKA: {office_home_clipart_results['cka']*100:.2f}")
    print(f"Office-Home Product CKA: {office_home_product_results['cka']*100:.2f}")
    print(f"CUB CKA: {cub_results['cka']*100:.2f}")
    print(f"DomainNet-clipart CKA: {domainnet126_clipart_results['cka']*100:.2f}")
    print(f"DomainNet-painting CKA: {domainnet126_painting_results['cka']*100:.2f}")
    print(f"DomainNet-real CKA: {domainnet126_real_results['cka']*100:.2f}")
    print(f"DomainNet-sketch CKA: {domainnet126_sketch_results['cka']*100:.2f}")
