    ...
    if args.imagenet_arch:
        ...
        for i, data in enumerate(distill_loader):
            ...
            for name, module in model.named_modules():
                if isinstance(module, torch.nn.Sequential) and name.startswith('layer'):
                    hooks.append(module.register_forward_hook(hook_fn))
            for name, module in original_model.named_modules():
                if isinstance(module, torch.nn.Sequential) and name.startswith('layer'):
                    hooks_t.append(module.register_forward_hook(hook_fn_t))
            ...
            for f_s, f_t in zip(features_s, features_t):
                f_s_flat = f_s.view(f_s.size(0), f_s.size(1), -1)
                f_t_flat = f_t.view(f_t.size(0), f_t.size(1), -1)
                
                G_s = torch.bmm(f_s_flat, f_s_flat.transpose(1, 2))
                G_t = torch.bmm(f_t_flat, f_t_flat.transpose(1, 2))
                
                similarity_loss += F.mse_loss(G_s, G_t) / (G_s.size(-1) * G_s.size(-2))
