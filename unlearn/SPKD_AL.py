...
@iterative_unlearn
def SPKD_AL(data_loaders, model, criterion, optimizer, epoch, args, mask=None):
    # store initial model state at the beginning of unlearning (epoch 0)
    if not hasattr(SPKD_AL, 'original_model'):
        SPKD_AL.original_model = copy.deepcopy(model)
        SPKD_AL.original_model.eval()
        for param in SPKD_AL.original_model.parameters():
            param.requires_grad = False
    
    original_model = SPKD_AL.original_model
