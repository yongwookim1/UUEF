...
@iterative_unlearn
def PCU(data_loaders, model, criterion, optimizer, epoch, args, mask=None):
    ...
    emb_proj = ProjectionHead(feature_dim, 512, args.num_classes, num_layers=3).to(device)
    proxy_proj = ProjectionHead(feature_dim, 512, args.num_classes, num_layers=1).to(device)
