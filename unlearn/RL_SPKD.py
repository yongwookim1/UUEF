...
@iterative_unlearn
def RL_SPKD(data_loaders, model, criterion, optimizer, epoch, args, mask=None):
    # store initial model state at the beginning of unlearning (epoch 0)
    if not hasattr(RL_SPKD, 'original_model'):
        RL_SPKD.original_model = copy.deepcopy(model)
        RL_SPKD.original_model.eval()
        for param in RL_SPKD.original_model.parameters():
            param.requires_grad = False
    
    original_model = RL_SPKD.original_model
