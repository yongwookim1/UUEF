...
@iterative_unlearn
def PL_SPKD(data_loaders, model, criterion, optimizer, epoch, args, mask=None):
    # store initial model state at the beginning of unlearning (epoch 0)
    if not hasattr(PL_SPKD, 'original_model'):
        PL_SPKD.original_model = copy.deepcopy(model)
        PL_SPKD.original_model.eval()
        for param in PL_SPKD.original_model.parameters():
            param.requires_grad = False
    
    original_model = PL_SPKD.original_model
