...
@iterative_unlearn
def GA_CKA(data_loaders, model, criterion, optimizer, epoch, args, mask=None):
    # store initial model state at the beginning of unlearning
    if not hasattr(GA_CKA, 'original_model'):
        GA_CKA.original_model = copy.deepcopy(model)
        GA_CKA.original_model.eval()
    for param in GA_CKA.original_model.parameters():
        param.requires_grad = False
        
    original_model = GA_CKA.original_model
